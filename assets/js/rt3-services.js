angular.module('rezTrip')
            ...
            else
            {
                ...
                angular.forEach(self.roomsList, function(room, key ){
                    ...
                    if(room.min_average_price[0] != null && !this.isRate){
                       ...
                       this.isRate = true;
                       self.toNightsRate = "USD "+Math.round(roomRate);
                    ...
                    if(roomRate == null){
                      ...
                      todayRate = {'todayRate': "USD "+Math.round(roomRate)};
  ...
  .service('rt3SpecialRates', ['$rootScope', '$q', '$location','rt3api','$filter', function($rootScope, $q, $location, rt3api,$filter) {
    var specialRates = {
      ...
      sRdetail: {},
      locationHash: window.location.hash.substr(1).replace("%2F","") //$location.search().name || null
      ...
      rt3api.getAllSpecialRates().then(function(response) {
             ...
             $rootScope.$applyAsync(function() {
              ...
              if(specialRates.locationHash){
                  angular.forEach(formatResponseValue.special_rates, function(value, key) {
    ...
    function formatRespone(response) {
      if(rt3api.config.defaultLocale != 'en') {
        response.special_rates = response.special_rates.filter(function(offer) {
          return offer.rate_plan_code != 'PGARD'
        });
      }
  ...
  .service('rt3RoomDetails', ['$rootScope', '$q', '$location', 'rt3Search', 'rt3api', '$timeout', function($rootScope, $q, $location, rt3Search, rt3api, $timeout) {
    function RoomDetails() {
      ...
      params = {};
      brg = {};
      locationHash = $location.path().substr(1).replace("%2F","");
    }
