        ...
        $timeout(function() {
           ...
           for(var j= 0 ; j < roomsList.length ; j++){
             ...
             tmpName = $filter('formatNameForLink')(roomsList[j].name);
               ...
               if(rName == tmpName || roomId.toLowerCase() == roomsList[j].code.toLowerCase()){
                  ...
                  $scope.selectedRoom = selectedRoom = roomsList[j];
    ...
    .controller('offerDetail', ['$scope', 'rt3Search', 'rt3Browser','$timeout','$filter', function($scope, rt3Search, rt3Browser,$timeout,$filter) {
      window.onhashchange = function() {
        ...
        $timeout(function() {
           ...
           var oName = window.location.hash.substr(1).replace("%2F",""); //$("#offerId").val();
           var tmpName;
